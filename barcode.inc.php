	...
	*/
	...
	class BARCODE
	{
		...
		function _BARCODE($encoding="EAN-13")
		{
		    ...
		    $this->_font=$whereami."/arialbd.ttf";
		...
		function genBarCode($barnumber,$format="gif",$file="")
		{
			...
			elseif($this->_encode=="UPC-A")
			{
			 //mod scriptman
				...
				//if(strlen($barnumber)>12)
				{
				 	//$this->_error="Barcode number must be less then 13 characters."; vinamn
				 	$this->_error="UPC-A Barcode number must be 12 characters.";
					return false;
				}
				$this->_eanBarcode($barnumber,$this->_scale,$file);
			}
			...
			{
			 	//if(strlen($barnumber)>12) scriptman
				if(strlen($barnumber) != 8)
				{
					$this->_error="UPC-E Barcode number must be 8 characters.";
					return false;
				}
...
function _upcEcheckDigit($barnumber,$number) //scriptman new
		{
			...
			//mod scriptman. yes I am going to use a global variable. tsk tsk.
			//originally they just padded upc-e codes with zeros on the left below. big bug!!!!!
			if ( ($this->_encode=="UPC-E") and (strlen($barnumber) == 8) )
				{
